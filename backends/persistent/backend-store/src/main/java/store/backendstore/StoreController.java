...
public class StoreController {
    ...
    @GetMapping("/getcart")
    public ResponseEntity<List<Cart>> GetCartUser(@Valid @RequestBody GetCartRequest request) {
        List<Cart> booksList = service.GetCart(request.usuario());
    ...
    public ResponseEntity<?> BuyCart(@Valid @RequestBody BuyCartRequest request) {
        ...
        var SendMsj = rabbitMQSender.send(new MsjBroker(request.usuario(), carrito));
        if (SendMsj) {
